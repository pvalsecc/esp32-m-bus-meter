...
class DlmsTest : public testing::Test {
  public:
    static void staticHandleDlmsPacket(void *arg, const uint8_t *bytes, int len) {
        reinterpret_cast<DlmsTest *>(arg)->handleDlmsPacket(bytes, len);
    }

    void handleDlmsPacket(const uint8_t *bytes, int len) {
        _packets.push_back(std::vector<uint8_t>(bytes, bytes + len));
    }
...
TEST_F(DlmsTest, sample) {
    const std::vector<uint8_t> packet1 =
        hex2vector("E6E700E0 40 0001 0000 70 0F00 4B10AB0C 07E80A1B 0707232D FF800000 020D010D 02041200 28090600 "
    ...
    const std::vector<uint8_t> packet2 =
        hex2vector("E0       40 0002 0000 6C 02 04120003 09060101 010800FF 0F021200 00020412 00030906 01010208 "
    ...
    const std::vector<uint8_t> packet3 =
        hex2vector("E0       C0 0003 0000 5C 02 04120003 09060100 340700FF 0F021200 00020412 00030906 01004807 "
    ...
    ASSERT_TRUE(dlms_decode(state, packet1.data(), packet1.size()));
    EXPECT_EQ(_packets.size(), 0);
    ASSERT_TRUE(dlms_decode(state, packet2.data(), packet2.size()));
    EXPECT_EQ(_packets.size(), 0);
    ASSERT_TRUE(dlms_decode(state, packet3.data(), packet3.size()));
