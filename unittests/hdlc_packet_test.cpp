...
TEST(Hdlcpacket, type1) {
    ...
    Buffer decoded;
    ASSERT_TRUE(hdlc_packet_decode(&data, &decoded));
    EXPECT_EQ(decoded.len, 0x7D - 8 - 2);
    EXPECT_EQ(memcmp(decoded.bytes, data.bytes + 8, decoded.len), 0);
}

TEST(Hdlcpacket, type2) {
    const auto data =
        hex2buffer("A06D CEFF 03 13 90F1 E0C00003 00005C02 04120003 09060100 340700FF 0F021200 00020412 00030906 "
    ...
    Buffer decoded;
    ASSERT_TRUE(hdlc_packet_decode(&data, &decoded));
    EXPECT_EQ(decoded.len, 0x6D - 8 - 2);
    EXPECT_EQ(memcmp(decoded.bytes, data.bytes + 8, decoded.len), 0);
}

TEST(Hdlcpacket, type3) {
    Buffer data = hex2buffer(
    ...
    Buffer decoded;
    ASSERT_TRUE(hdlc_packet_decode(&data, &decoded));
    EXPECT_EQ(decoded.len, 0x84 - 8 - 2);
    EXPECT_EQ(memcmp(decoded.bytes, data.bytes + 8, decoded.len), 0);
}
